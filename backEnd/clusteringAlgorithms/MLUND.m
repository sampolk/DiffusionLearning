function Clusterings = MLUND(X, Hyperparameters, G, p)
%{
...
Inputs: X:                      Data matrix.
        ...
        G:                      Graph structure computed using  
                                extractGraph.m
        p:                      Kernel Density Estimator computed using
                                KDE.m
...
if nargin == 2
    ...
    p  = extractKDE(Dist_NN, Hyperparameters);
end
...
if isreal(T)
    ...
    if n_J > 0 % There is a nontrivial clustering of X.
        [~,VI_tot,t] = totalVIMinimization(Ct, Kt);
